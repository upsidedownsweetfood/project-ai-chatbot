...
pub fn stream_audio_data(file_path: String) {
    ...
    let file = File::open(file_path).expect("Impossible to open MP3 file");
    let source = rodio::Decoder::new(BufReader::new(file)).expect("Impossible to decode MP3 file");
    ...
    match OutputStreamBuilder::open_default_stream() {
        ...
        Err(e) => {
            println!("OutputStreamBuilder failed: {}", e);
        }
    }
}
...
pub fn play_mp3_file() {
        ...
        Ok(devices) => {
            ...
            for (i, device) in devices.enumerate() {
                println!(
                    "  {}: {}",
                    i,
                    device.name().unwrap_or("Sconosciuto".to_string())
                );
            }
        }
    ...
    println!(
        "Decodificato: {} samples, {}Hz, {} canali",
        audio_data.len(),
        sample_rate,
        channels
    );
